...
def predict(data):
    """ Get 2 colums from `df` and returns the predicted ouput as a column (pd.Series)."""
...
def main(input_dir, output_dir):
    ...
    assert isinstance(
        predicted_result, pd.core.series.Series
    ), f"Invalid predicted output type {type(predicted_result)}"

    predicted_result.to_csv(
        os.path.join(output_dir, "data.predict"), index=False, header=False
    )
